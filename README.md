...
- 普通 `gt del <分支名>` 在本地分支不存在时会报错；如果检测到 `origin/<分支名>` 仍存在，会提示使用 `gt del -a <分支名>` 同时删除远端分支。`gt del -a <分支名>` 会跳过不存在的一侧，只有本地和远端都不存在时才报错。
...
`git-merge` 用于基于远端基线分支创建或重建一个本地目标分支，并把另一个远端分支的最新内容合并进来。传 2 个参数时，会按最新 `origin/a` 重建本地 `a`，再把最新 `origin/b` 合并到本地 `a`；传 3 个参数时，会先删除本地 `b`、`c`，再按最新 `origin/c` 创建本地 `b`，最后把最新 `origin/a` 合并到本地 `b`。

### 原理描述

这个脚本的核心原理是把远端分支当成可信输入源，而不是依赖当前本地分支状态。它会先同步远端引用；传 3 个参数时会先清理本地目标分支和基线分支，再按远端基线创建本地目标分支，最后把指定来源远端分支合并进目标分支。这样可以避免“当前本地分支不干净、落后，或者已经偏离远端”导致的基线不一致问题。
...
9. 传 2 个参数时执行 `git checkout --detach origin/a`；传 3 个参数时先执行 `git checkout --detach origin/c`，使本地 `b`、`c` 均可删除。
10. 传 3 个参数时，强制删除存在的本地 `b`、`c`；传 2 个参数时不删除额外分支。
11. 传 2 个参数时执行 `git checkout -B <本地目标分支>` 创建或重建目标分支；传 3 个参数时执行 `git checkout --no-track -b b origin/c` 创建目标分支。
12. 执行 `git merge --no-edit <远端来源分支>`；传 2 个参数时合并 `origin/b`，传 3 个参数时合并 `origin/a`。
