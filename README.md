...
./gt merge <远端来源分支a> <本地目标分支b> <远端基线分支c>
./gt gam <源分支> <目标分支>
```
...
- 删除本地分支会先使用 `git branch -d`；如果 Git 判断该分支存在未合并提交，会再次确认是否使用 `git branch -D` 强制删除。
...
`git-merge` 用于基于远端基线分支创建或重建一个本地目标分支，并把另一个远端分支的最新内容合并进来。传 2 个参数时，会按最新 `origin/a` 重建本地 `a`，再把最新 `origin/b` 合并到本地 `a`；传 3 个参数时，如果本地 `a` 没有未推送提交，会先删除本地 `b`、`c`，再按最新 `origin/c` 创建本地 `b`，最后把最新 `origin/a` 合并到本地 `b`。

### 原理描述

这个脚本的核心原理是把远端分支当成可信输入源，而不是依赖当前本地分支状态。它会先同步远端引用；传 3 个参数时，会先确认本地来源分支没有未推送到远端来源分支的提交，再清理本地目标分支和基线分支，按远端基线创建本地目标分支，最后把指定来源远端分支合并进目标分支。这样可以避免“当前本地分支不干净、落后，或者已经偏离远端”导致的基线不一致问题。
...
第二条命令表示会先删除本地 `feature/test`、`release`，再按最新 `origin/release` 创建本地 `feature/test`，然后把最新 `origin/local` 合并进来。

### 执行流程

1. 校验参数是否完整，只允许 2 个或 3 个业务参数。
2. 如果传 2 个参数，则 `a` 是远端基线分支、`b` 是远端来源分支，本地目标分支使用 `a`；如果传 3 个参数，则 `a` 是远端来源分支、`b` 是本地目标分支、`c` 是远端基线分支。
3. 传 3 个参数时，校验本地目标分支名 `b` 不与远端来源分支 `a`、远端基线分支 `c` 同名。
4. 校验当前目录是否为 Git 仓库。
5. 校验已跟踪文件没有未提交改动。
6. 执行 `git fetch --all --prune`，同步远端引用。
7. 校验远端基线分支和远端来源分支都存在；如果来源远端分支不存在，脚本会直接失败。
8. 传 3 个参数时，校验远端目标分支不存在；如果 `origin/b` 已存在，脚本会直接失败。
9. 传 3 个参数时，如果本地 `a` 存在未推送到 `origin/a` 的提交，直接失败，不删除任何本地分支。
10. 传 2 个参数时执行 `git checkout --detach origin/a`；传 3 个参数时先执行 `git checkout --detach origin/c`，使本地 `b`、`c` 均可删除。
11. 传 3 个参数时，强制删除存在的本地 `b`、`c`；传 2 个参数时不删除额外分支。
12. 传 2 个参数时执行 `git checkout -B <本地目标分支>` 创建或重建目标分支；传 3 个参数时执行 `git checkout --no-track -b b origin/c` 创建目标分支。
13. 执行 `git merge --no-edit <远端来源分支>`；传 2 个参数时合并 `origin/b`，传 3 个参数时合并 `origin/a`。

### 当前分支影响

当前本地分支是否为远端基线分支，不影响最终结果。脚本不会先依赖同名本地分支，而是直接以远端最新基线分支为准创建或重建本地目标分支。

如果当前分支正好是远端基线分支对应的本地分支，脚本仍然会先以对应的远端基线进入 detached HEAD。传 3 个参数时，随后会删除本地 `b`、`c`；传 2 个参数时仅重建本地目标分支。本地未 push 的提交不会自动进入目标分支。
...
- 传 3 个参数时，如果本地 `a` 存在未推送到 `origin/a` 的提交，脚本会直接失败，不会删除本地 `b`、`c`。
- 传 3 个参数时，本地 `b`、`c` 存在即会被强制删除；其中的未 push 提交会丢失。
- 传 2 个参数时，如果本地目标分支已存在，脚本会按最新远端基线分支重建它。
- 合并远端来源分支时如果出现冲突，脚本会停止，需要手工解决冲突。
- 传 3 个参数时，创建后的本地 `b` 不跟踪 `origin/c`；如果后续需要建立 `origin/b` 跟踪关系，需要在合适时机手工执行 `git push -u origin b`。
